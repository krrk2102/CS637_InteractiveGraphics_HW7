...
{   
    ...
    glBufferData( GL_ARRAY_BUFFER, render_vertices.size()*sizeof(vec4)+render_normals.size()*sizeof(vec4)+control_points.size()*sizeof(vec4)+control_points_color.size()*sizeof(color4), NULL, GL_STATIC_DRAW );
    ...
    glBindAttribLocation( basic, 5, "vVertex" );
    glBindAttribLocation( basic, 6, "vColor" );
    LinkShader( basic );
    ...
    glVertexAttribPointer( 6, 4, GL_FLOAT, GL_FALSE, 0,
                           BUFFER_OFFSET( render_vertices.size()*sizeof(vec4)+render_normals.size()*sizeof(vec4)+control_points.size()*sizeof(vec4) ) );
}
...
initData( void )
{
    ...
    glBufferSubData( GL_ARRAY_BUFFER, render_vertices.size()*sizeof(vec4)+render_normals.size()*sizeof(vec4), control_points.size()*sizeof(vec4), control_points.data() );
    glBufferSubData( GL_ARRAY_BUFFER, render_vertices.size()*sizeof(vec4)+render_normals.size()*sizeof(vec4)+control_points.size()*sizeof(vec4), control_points_color.size()*sizeof(color4), control_points_color.data() );
}
...
recal( void )
{
    ...
    GLint loc_phong = glGetUniformLocation( program, "phong" );
    glUniform1i( loc_phong, phong );
...
//----------------------------------------------------------------------------
void
MainSubMenuPhong( int id )
{
    switch ( id ) {
        case 1: phong = true; break; // Switch to phong shading.
        case 2: phong = false; break; // Switch to gouraud shading.
    }
    glutPostRedisplay();
    cout<<"glutPostRedisplay for gouraud/phong shading switching."<<endl;
}
...
setMainWinMenu( void )
{
    int submenu_id_r, submenu_id_p, submenu_id_s, submenu_id_m, submenu_id_l;
    ...
    // Create submenu for shading switch.
    submenu_id_s = glutCreateMenu( MainSubMenuPhong );
    glutAddMenuEntry( "Phong Shading", 1 );
    ...
    glutAddSubMenu( "Shading", submenu_id_s );
...
{   
    ...
    MainWindow = glutCreateWindow( "ICG_hw7" ); // Initializing & setting main window.
